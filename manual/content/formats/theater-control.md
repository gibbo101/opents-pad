...
Artwork for a set is named from its [`FileName`](/keys/filename/) plus a two-digit index starting at `01`, extended with the theater's [`Suffix`](/keys/suffix/): `FileName=RVCLIF` with `TilesInSet=8` loads `RVCLIF01.TEM` through `RVCLIF08.TEM` in the temperate theater, and the `.SNO` equivalents in snow. A trailing lowercase letter marks an alternate for the same tile — `RVCLIF01a`, then `RVCLIF01b` — and reading alternates stops at the first letter with no file. When a theater file is missing, the loader retries the same name with the theater's [`MMSuffix`](/keys/mmsuffix/) — `.MMT` in temperate and `.MMS` in snow — unless the set carries [`NonMarbleMadness=0`](/keys/nonmarblemadness/).
