...
The scheme decides which screen appears, not how a screen behaves. Under `Controller` the skirmish setup, new campaign, and options screens are console-style lists that edit the same settings as the dialogs, with the map list numbered and a shoulder button or Shift stepping five at a time; every other screen is still the original dialog until it gains a console-style version. Under `Controller` the shell menus also render at 640x400, the size their artwork was drawn for, and the configured resolution applies once a game starts. Under `KeyboardMouse` nothing differs from a build without this setting.

A name the game does not recognize leaves the setting at `KeyboardMouse`. The setting changes from either scheme: the options page has a Switch To Controller button, enabled while a controller is connected, and the console-style options screen has a Control Scheme row offering all three values. A choice made on either screen is saved as that scheme, so `Auto` only returns by choosing it on the console screen or writing it in the file. The change takes effect at the next screen.
