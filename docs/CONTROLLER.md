...
| Play, top bar | `code/tab.cpp`, `code/display.cpp`, `code/mainopt.cpp`, `code/gscreen.cpp` | Under the controller scheme there is no top bar: the map takes the full height, and the two tabs that matter are drawn over the map's top corners after its foreground pass, from the screen renderer, since the map covers those corners. At the left, the Options tab on the sidebar's tab art with the pad's menu button glyph beside its label, so it reads as the prompt it is; a click on it opens the options as the bar's tab did. At the right, the mission timer while one runs. The sidebar panel is the pure five-row panel with the credits at its top. The keyboard scheme keeps the bar as it was. |
...
1. The top bar is done: the map takes the full height, the Options tab
   sits over the map's top left corner with the pad's menu glyph, the timer
   over the top right, and the credits stay in the sidebar's strip. An
   Options cell across the sidebar's top was tried first and dropped: the
   pad has a button for it, so the cell only cost sidebar height.
