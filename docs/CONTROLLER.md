...
| Play, commands | `code/gamepad.cpp` | The chords are in [On the map](CONTROLS.md#on-the-map), [Teams](CONTROLS.md#teams) and [Waypoints](CONTROLS.md#waypoints). Each button runs the engine's own named command, so the keyboard bindings stay the source of the behavior. The hold-to-select counts a unit as a combat unit when it carries a weapon or gains one by deploying, and each of its four selections announces itself in the message list. |
