...
| Play, sidebar slide | `code/sidebar.cpp`, `code/video.cpp`, `code/gamepad.cpp`, `code/tab.cpp` | Under the controller scheme the sidebar starts away from the map, in the manner of Retaliation: the map takes the whole width under the bar and the credits sit at the bar's right end. Triangle slides the panel in over the map and the bar, 200 ms from the presenter, with the pad on it; triangle again, or anything else that lets the pad go such as placing a building, slides it out. R1 with triangle slides it in sticky, so the pad can leave it and it stays; R1 with triangle again lets it go. The sticky state is saved as `PadSidebarSticky` in `SUN.INI` and a sticky panel comes back on the next scenario's first tick, the pad staying on the map. Nothing else opens it, as in C&C1 on the PlayStation, where deploying the MCV leaves the panel away until the player calls it. While the panel is fully in it sits beside the map as before, the map refitted to the width left, the refit done the moment the panel covers that strip so nothing shows; on the way out the map takes the width back first, under the panel, then the panel goes. The presenter keeps the slide as a share of the panel's width and draws the panel last, over the map and the bar; the frame's sidebar columns take no screen width while the panel is away, and the timer's tab keeps left of whatever covers the bar's end. The main loop settles a finished slide beside the zoom step. |
...
1. The top bar is done, C&C1 style: the bar stays, at a constant size like
   ...
   the bar as Retaliation puts the money over the map. Done since: the
   sidebar slides, hidden by default, triangle in and out, R1 with
   triangle sticky and remembered. Luke checked C&C1 on the PlayStation:
   deploying an MCV does not bring the panel up outside sticky mode, so
   an auto-open at a base start and on MCV deploy was built and dropped.
