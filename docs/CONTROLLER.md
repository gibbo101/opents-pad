...
| Load Mission | `code/loaddlg.cpp` | Console list of the save games, newest first, date at the left and description at the right, a star for multiplayer saves; long lists scroll. Square, the Delete key, or a click on its prompt deletes the focused save once a No/Yes row under the save's name is set to Yes, and the list is read again from the folder. |
...
4. Later: the display work in the direction notes.
