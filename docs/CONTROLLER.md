...
| Audio | `code/consoleoptions.cpp` | Console screen from Options and the pause menu: music, sound and voice volume stepped live, and in play shuffle, repeat, a Track row that steps through the songs by name with the playing one marked and A on the row playing it, and Stop Music. Accept keeps the volumes and saves; back restores them. |
