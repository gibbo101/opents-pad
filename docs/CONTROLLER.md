...
| In-game menu | `code/consoleingame.cpp` | Under the controller scheme the Escape menu in play is a console screen of its own, like the main menus: the render size drops to 640x400 on entry and returns to the play size on exit, so the text and artwork match the shell on every panel. "Game Paused" above a box sized to its rows, in the menu pages' colours, on the mission's briefing plate rather than the shell backdrop, focus starting on Return To Mission. Game Options opens the console options screen without its display rows or the campaign difficulty, which the original in-game dialog never applied either; Audio Options opens the audio screen; Mission Briefing restates; Save Game writes a fresh slot under the mission's name, reports beneath the box, and posts the message the quick save posts for when play resumes; Load Game opens the console load list; Restart Mission and Abort Mission each confirm first. In a LAN game Save and Load go through the multiplayer paths and Restart becomes Surrender. The menu loop's pause, resume, restart and exit handling is shared with the dialog. |
