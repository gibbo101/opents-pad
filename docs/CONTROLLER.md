...
| Play, top bar | `code/tab.cpp`, `code/display.cpp`, `code/mainopt.cpp`, `code/gscreen.cpp` | Under the controller scheme there is no top bar: the map takes the full height, and the Options tab moves to the top of the sidebar, drawn on the sidebar's own tab art at the sidebar's width above the credits, so the sidebar panel is 16 taller and everything on it sits 16 lower. The tab is a cell of the pad's panel: up from the radar, or from the mode buttons while the radar is off, reaches it, cross opens the options as the tab's click does, and the pad's outline marks it; a mouse click on it works too. The mission timer's tab is drawn in the map's top right corner after the map's foreground pass, from the screen renderer, since the map now covers that corner. The keyboard scheme keeps the bar as it was. |
...
1. The top bar is done: the map takes the full height, the timer is a map
   corner overlay, the credits stay in the sidebar's strip, and the
   controller's Options cell is the tab across the sidebar's top. The
   mouse's Options tab stays where it is under the keyboard scheme.
