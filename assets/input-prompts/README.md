...
| --- | --- | --- |
| `xbox_` | Xbox Series | the `xbox_button_color_*` set |
| `playstation_` | PlayStation Series | the `playstation_button_color_*` set |
| `deck_` | Steam Deck | the `steamdeck_button_*` set |

`accept`, `back`, `third`, and `fourth` are A, B, X, Y on an Xbox pad or the
Deck and cross, circle, square, triangle on a PlayStation pad.
