...
class Tactical : public AbstractClass
{
	...
	public:
		...
		*/
		...
		// Where the view was when the band began, so a scroll leaves the band's anchor on the map.
		Point2D RubberBandView;
		// View pixels at the right hidden under a panel, which the view may scroll past the
		// map's edge by.
		int ViewCoveredRight = 0;
