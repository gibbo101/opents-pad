...
OptionsClass::OptionsClass(void) :
	...
	PadZoomWidth(0),
	PadZoomHeight(0),
	PadSnap(PAD_SNAP_DEFAULT),
	PadSidebarSticky(false),
	SoundVolume(.7f),
	VoiceVolume(1.0f),
	ScoreVolume(.5f),
	AutoScroll(true),
...
void OptionsClass::Load_Settings(void)
{
	...
	*/
	...
	PadSnap = std::clamp(ConfigINI.Get_Int("Options", "PadSnap", PadSnap), 0, int(PAD_SNAP_MAX));
	PadSidebarSticky = ConfigINI.Get_Bool("Options", "PadSidebarSticky", PadSidebarSticky);
...
void OptionsClass::Save_Settings (void)
{
	...
	*/
	...
	ConfigINI.Put_Int("Options", "PadSnap", PadSnap);
	ConfigINI.Put_Bool("Options", "PadSidebarSticky", PadSidebarSticky);
